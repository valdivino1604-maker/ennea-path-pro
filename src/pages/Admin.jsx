...
function buildCompletedAnswers(answers) {
  ...
  QUESTIONS.forEach((question) => { if (completed[question.id] == null) completed[question.id] = 3; });
  return completed;
}
...
export default function Admin() {
  ...
  const companyRows = useMemo(() => {
    ...
    results.forEach((result) => { const row = ensure(result.participant_company); row.completed += 1; if (result.participant_role) row.roles.add(normalizeText(result.participant_role)); });
  ...
  async function saveEdit() {
    ...
    try {
      ...
      const relatedParticipants = participants.filter((participant) =>
      ...
      if (target.kind === "result") {
        await base44.entities.TestResult.update(target.id, { participant_name: newName, participant_company: newCompany });
      }
      if (target.kind === "participant") {
        await base44.entities.TestParticipant.update(target.id, { full_name: newName, company: newCompany });
      }

      await Promise.all([
        ...relatedResults
          .filter((result) => !(target.kind === "result" && result.id === target.id))
          .map((result) => base44.entities.TestResult.update(result.id, { participant_name: newName, participant_company: newCompany }).catch(() => null)),
        ...relatedParticipants
          .filter((participant) => !(target.kind === "participant" && participant.id === target.id))
          .map((participant) => base44.entities.TestParticipant.update(participant.id, { full_name: newName, company: newCompany }).catch(() => null))
      ]);
      ...
      setResults((rows) => rows.map((result) => {
        ...
        return related ? { ...result, participant_name: newName, participant_company: newCompany } : result;
      }));
      setParticipants((rows) => rows.map((participant) => {
        ...
        return related ? { ...participant, full_name: newName, company: newCompany } : participant;
      }));
  ...
  return (
    <div className="h-screen bg-background flex overflow-hidden">
      <aside className="hidden lg:flex flex-col w-60 bg-sidebar border-r border-sidebar-border shrink-0">
        <div className="p-5"><Link to="/" className="flex items-center gap-2.5"><div className="w-8 h-8 rounded-lg bg-primary/20 flex items-center justify-center text-sm font-bold text-primary">EP</div><span className="font-display text-base font-semibold text-foreground">Diagnostico de Lideranca</span></Link></div>
        <nav className="flex-1 px-3 space-y-0.5">{NAV_ITEMS.map((item) => <Link key={item.label} to={item.href} className={`flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm ${item.href === "/admin" ? "bg-primary/15 text-primary font-medium" : "text-muted-foreground hover:text-foreground hover:bg-white/[0.04]"}`}><item.icon className="w-4 h-4" /> {item.label}</Link>)}</nav>
      </aside>
      ...
      <main className="flex-1 flex flex-col min-w-0 overflow-y-auto">
        <header className="sticky top-0 z-10 bg-background/80 backdrop-blur-xl border-b border-border px-4 sm:px-6 py-3 flex items-center gap-4">
          ...
          <div className="flex-1"><h1 className="font-display text-lg font-bold text-foreground">Dashboard de Analise</h1><p className="text-xs text-muted-foreground">Resultados, pendencias e edicao de dados</p></div>
        ...
        <div className="p-4 sm:p-6 space-y-6">
          ...
          <section className="bg-card border border-border rounded-2xl p-4 space-y-3">
            <div className="flex flex-col lg:flex-row gap-3">
              <div className="flex items-center gap-2 flex-1"><Search className="w-4 h-4 text-muted-foreground" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Buscar por nome, e-mail, empresa, funcao/cargo ou telefone..." className="bg-background" /></div>
              <select value={filterCompany} onChange={(event) => setFilterCompany(event.target.value)} className="h-10 rounded-lg bg-background border border-border px-3 text-sm text-foreground"><option value="all">Todas as empresas</option>{companyOptions.map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select>
              <select value={filterType} onChange={(event) => setFilterType(event.target.value)} className="h-10 rounded-lg bg-background border border-border px-3 text-sm text-foreground"><option value="all">Todos os tipos</option>{[1, 2, 3, 4, 5, 6, 7, 8, 9].map((type) => <option key={type} value={type}>Tipo {type} - {TYPE_NAMES[type]}</option>)}</select>
          ...
          <section className="bg-card border border-border rounded-2xl overflow-hidden">
            <div className="p-4 border-b border-border"><h2 className="font-heading text-sm font-semibold text-foreground">Analise por empresa</h2><p className="text-xs text-muted-foreground">Resumo por empresa, funcoes, concluidos e pendentes</p></div>
            <div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b border-border"><Th>Empresa</Th><Th>Funcoes</Th><Th>Total</Th><Th>Concluidos</Th><Th>Pendentes</Th></tr></thead><tbody>{companyRows.map((row) => <tr key={row.key} className="border-b border-border/50"><Td>{row.company}</Td><Td>{row.roleCount}</Td><Td>{row.total}</Td><Td>{row.completed}</Td><Td>{row.pending}</Td></tr>)}</tbody></table></div>
          </section>
          ...
          <section className="bg-card border border-border rounded-2xl overflow-hidden">
            ...
            <ClarityLegend />
            <div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b border-border"><Th>Participante</Th><Th>Empresa</Th><Th>Funcao/Cargo</Th><Th>Tipo</Th><Th>Clareza</Th><Th>Data</Th><Th align="right">Acao</Th></tr></thead><tbody>
              {filteredResults.length === 0 ? <tr><td colSpan={7} className="text-center p-10 text-sm text-muted-foreground">Nenhum resultado concluido encontrado.</td></tr> : filteredResults.map((result) => (
                <tr key={result.id} className="border-b border-border/50 hover:bg-white/[0.02]">
                  ...
                  <Td><p>{result.participant_company || "—"}</p><button onClick={() => openEditor({ kind: "result", id: result.id, participant_id: result.participant_id, email: result.participant_email, name: result.participant_name, company: result.participant_company })} className="text-[11px] text-primary hover:underline">Editar dados</button></Td>
                  <Td>{result.participant_role || "—"}</Td><Td><TypeBadge type={result.dominant_type} /></Td><Td><ConfidenceBadge value={result.confidence_level} /></Td><Td>{formatDate(result.created_date)}</Td>
          ...
          <section className="bg-card border border-border rounded-2xl overflow-hidden">
            <div className="p-4 border-b border-border flex items-center justify-between gap-3"><div><h2 className="font-heading text-sm font-semibold text-foreground">Candidatos cadastrados sem resultado</h2><p className="text-xs text-muted-foreground">Use Gerar resultado quando o teste travar no fim. Com 89/90, a pergunta faltante entra como neutra.</p></div><span className="text-xs text-muted-foreground">{filteredPending.length} encontrado(s)</span></div>
            <div className="overflow-x-auto"><table className="w-full"><thead><tr className="border-b border-border"><Th>Candidato</Th><Th>Empresa</Th><Th>Funcao/Cargo</Th><Th>Progresso</Th><Th>Status</Th><Th align="right">Acao</Th></tr></thead><tbody>
              {filteredPending.length === 0 ? <tr><td colSpan={6} className="text-center p-10 text-sm text-muted-foreground">Nenhum cadastro pendente encontrado.</td></tr> : filteredPending.map((participant) => {
                const answered = answerCount(participant.answers); const ready = canGenerateResult(participant);
                return <tr key={participant.id} className="border-b border-border/50 hover:bg-white/[0.02]"><Td><p className="font-medium text-foreground">{participant.full_name || "Sem nome"}</p><p className="text-[11px] text-muted-foreground">{participant.email || "—"}</p></Td><Td><p>{participant.company || "—"}</p><button onClick={() => openEditor({ kind: "participant", id: participant.id, email: participant.email, name: participant.full_name, company: participant.company })} className="text-[11px] text-primary hover:underline">Editar dados</button></Td><Td>{participant.role || "—"}</Td><Td><span className={ready ? "text-emerald-300" : "text-amber-300"}>{answered}/{QUESTIONS.length}</span></Td><Td>{ready ? <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-emerald-500/10 text-emerald-300 border border-emerald-500/20">Pronto para gerar</span> : <span className="text-[10px] font-bold px-2 py-0.5 rounded-md bg-amber-500/10 text-amber-300 border border-amber-500/20">Incompleto</span>}</Td><Td align="right"><div className="flex justify-end gap-1"><Link to={`/test/${participant.id}`}><Button variant="ghost" size="sm" className="h-8 gap-1 text-primary text-xs">Abrir</Button></Link>{ready && <Button size="sm" disabled={generatingId === participant.id} onClick={() => generateResultForParticipant(participant)} className="h-8 gap-1 text-xs"><FileCheck2 className="w-3 h-3" /> {generatingId === participant.id ? "Gerando..." : "Gerar resultado"}</Button>}<Button variant="ghost" size="sm" disabled={deletingId === participant.id} onClick={() => deleteCandidate({ kind: "participant", id: participant.id, email: participant.email, name: participant.full_name })} className="h-8 text-xs text-destructive">Excluir</Button></div></Td></tr>;
              })}
            </tbody></table></div>
          </section>
        </div>
      ...
      {editTarget && <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/70 px-4"><div className="w-full max-w-md rounded-2xl border border-border bg-card p-5 shadow-2xl"><div className="flex items-start justify-between gap-4 mb-4"><div><h2 className="font-heading text-base font-semibold text-foreground">Editar dados</h2><p className="text-xs text-muted-foreground mt-1">Corrija nome e empresa do candidato.</p></div><button onClick={() => setEditTarget(null)} className="text-muted-foreground hover:text-foreground"><X className="w-5 h-5" /></button></div><label className="text-xs text-muted-foreground">Nome</label><Input value={editNameValue} onChange={(event) => setEditNameValue(event.target.value)} className="mt-1 mb-3 bg-background" placeholder="Nome do candidato" autoFocus /><label className="text-xs text-muted-foreground">Empresa</label><Input value={editCompanyValue} onChange={(event) => setEditCompanyValue(event.target.value)} className="mt-1 bg-background" placeholder="Nome da empresa" /><div className="flex justify-end gap-2 mt-5"><Button variant="outline" onClick={() => setEditTarget(null)} disabled={savingEdit}>Cancelar</Button><Button onClick={saveEdit} disabled={savingEdit}>{savingEdit ? "Salvando..." : "Salvar dados"}</Button></div></div></div>}
